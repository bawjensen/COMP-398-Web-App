...
function saveFilters(filterObj) {
    Promise.resolve(filterObj).then(function saveToDatabase(filterObj) {
        MongoClient.connect('mongodb://bawjensen:dummytest@ds049160.mongolab.com:49160/webapp', function handleResponse(err, db) {
            if (err) { return console.error(err) };

            // db.hw14.save(filterObj);
            var hw14Collection = db.collection('hw14');
            hw14Collection.save(filterObj, function saveCallback() {
                console.log('yay');
            });
        });
    });
...
function saveYearOfData(year) {
    MongoClient.connect('mongodb://bawjensen:dummytest@ds049160.mongolab.com:49160/webapp', function handleResponse(err, db) {
        ...
        // db.hw14;
        var hw14Collection = db.collection('hw14');
        hw14Collection.save(yearData, function saveCallback() {
            console.log('yay for year');
        });
    });
}
...
function saveScheduleData() {
    ...
    //     Promise.resolve(scheduleData[year]).then(saveYearOfData);
    // }

    return Promise.all(
        Object.keys(scheduleData).map(function mapKeyToPromise(year, i, array) {
            return new Promise(function promisifyYearSaving(resolve, reject) {
                MongoClient.connect('mongodb://bawjensen:dummytest@ds049160.mongolab.com:49160/webapp', function handleResponse(err, db) {
                    if (err) { return console.error(err) };

                    // db.hw14;
                    var hw14Collection = db.collection('hw14');
                    hw14Collection.save(scheduleData[year], function saveCallback(err) {
...
function fetchAndParseAll() {
    ...
    //     // db.hw14.save(filterObj);
    //     var hw14Collection = db.collection('hw14');
    //     hw14Collection.remove({}, function handleWipeResponse(err) {
    //         console.log('Wiped');
    //     });
    // });
